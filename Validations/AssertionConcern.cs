...
public class AssertionConcern
{
    ...
    public static void AssertArgumentNotNull(string[] array, string message)
    {
        var arrayObj = array is not null && array.Length > 0 && array.All(x => !string.IsNullOrEmpty(x));

        if (!arrayObj)
            throw new InvalidOperationException(message);
    }
    public static void AssertArgumentNotNull(IFormFileCollection files, string message)
    {
        var isValid = files is not null && files.Count > 0 && files.All(x => !string.IsNullOrEmpty(x.FileName));
